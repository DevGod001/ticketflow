    ...
    try {
      ...
      if (currentUser.verified_organizations?.length > 0) {
        ...
        if (currentUser.active_organization_id) {
          const active = orgs.find(org => org.id === currentUser.active_organization_id);
          setActiveOrg(active);

          const canManage = active && (active.owner_email === currentUser.email || active.admins?.includes(currentUser.email));
    ...
    <div className="p-3 md:p-6 space-y-4 md:space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-slate-900">Organizations</h1>
            <p className="text-sm text-slate-600 mt-1 md:hidden">Manage your organizations</p>
          </div>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div className="overflow-x-auto">
          <TabsList className="bg-white border border-slate-200 shadow-sm w-full min-w-max">
            {userOrgs.length > 0 && (
              <TabsTrigger value="overview" className="data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 text-xs md:text-sm whitespace-nowrap">
                Overview
              </TabsTrigger>
            )}
            {activeOrg && canManageRequests && (
              <TabsTrigger value="manage" className="data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 text-xs md:text-sm whitespace-nowrap">
                Manage
              </TabsTrigger>
            )}
            {canManageRequests && (
              <TabsTrigger value="requests" className="data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 text-xs md:text-sm whitespace-nowrap">
                Join Requests {joinRequests.length > 0 && `(${joinRequests.length})`}
              </TabsTrigger>
            )}
            <TabsTrigger value="create" className="data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 text-xs md:text-sm whitespace-nowrap">
              Create New
            </TabsTrigger>
            <TabsTrigger value="join" className="data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 text-xs md:text-sm whitespace-nowrap">
              Join Existing
            </TabsTrigger>
          </TabsList>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 max-w-4xl mx-auto mt-8 md:mt-12">
            ...
            >
              <CardContent className="p-6 md:p-8 text-center">
                ...
                <h3 className="text-lg md:text-xl font-semibold text-slate-900 mb-2 md:mb-3">Create Organization</h3>
                <p className="text-slate-600 mb-4 md:mb-6 text-sm md:text-base">Start your team's journey by creating a new organization</p>
            ...
            >
              <CardContent className="p-6 md:p-8 text-center">
                ...
                <h3 className="text-lg md:text-xl font-semibold text-slate-900 mb-2 md:mb-3">Join Organization</h3>
                <p className="text-slate-600 mb-4 md:mb-6 text-sm md:text-base">Join an existing team using their organization ID</p>
        ...
        {userOrgs.length > 0 && (
          <TabsContent value="overview" className="space-y-4 md:space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
              {userOrgs.map((org) => (
                <Card key={org.id} className={`border-none shadow-lg hover:shadow-xl transition-all duration-300 ${
                  activeOrg?.id === org.id ? 'ring-2 ring-blue-500 bg-blue-50/30' : ''
                }`}>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs md:text-sm text-slate-500 truncate">ID: {org.organization_id}</p>
                        </div>
                      </div>
                  ...
                  <CardContent className="p-4 md:p-6 pt-0">
                    <p className="text-slate-600 mb-4 text-sm md:text-base line-clamp-2">{org.description}</p>
        ...
        {activeOrg && canManageRequests && (
          <TabsContent value="manage">
            <OrgManagement organization={activeOrg} onUpdate={loadOrganizationData} />
          </TabsContent>
        )}
